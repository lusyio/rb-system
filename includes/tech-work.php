...
if (isset($_POST["work"])) {
	...
	foreach ($arrayzap as $n => $i) { $c = 0; foreach ($i as $pr => $pp) { if ($c==0) {
...
<div class="d-flex mt-2">
	<button class="btn btn-light bg-white border mr-2" data-toggle="collapse" data-target="#addToplivo" aria-expanded="false" aria-controls="addToplivo"><i class="fas fa-tachometer-alt mr-3"></i>Внести показания топлива</button>
	<a href="/tech-sclad/" class="btn btn-light bg-white border mr-2"><i class="fas fa-tools mr-3"></i>Склад запчастей</a>
	<a href="/tech-to/" class="btn btn-light bg-white border mr-2"><i class="fas fa-clone mr-3"></i>Управление ТО</a>
	<button class="btn btn-light bg-white border mr-2" data-toggle="collapse" data-target="#addTech" aria-expanded="false" aria-controls="addTech"><i class="fas fa-plus mr-3"></i>Добавить технику</button>
</div>
</div>
			...
			<div class="col-sm-6">
				...
				<div class="form-group mb-2">
				  ...
				  <div class="form-group mb-2">
				    ...
				    <input type="number" name="toplivo" min="0" max="9999999999" class="form-control" placeholder="Топливо в литрах" autocomplete="off" required>
				  </div>
				  <div class="form-group">
				    <label for="staticEmail2">Моточасы</label>
				    <input type="number" name="motchas" min="0" max="9999999999" class="form-control" placeholder="Моточасы" autocomplete="off" required>
				  </div>
									 	...
									 	foreach ($i as $pr => $pp)
									 		{
										 		if ($c==0) {
											 		...
											 		$razn = $countzap - $pp;
											 		if ($razn < 0) { $textcolor = 'danger'; } else { $textcolor = 'success';}
	  ...
	  <div class="row mr-0">
			...
			<?php 
				...
				<div class="col-sm-3 pr-0">
					...
					<button type="submit" class="w-100 card btn p-0" style="white-space: normal">
						<div class="card-body p-1">
							<span style="white-space: nowrap;position: absolute;right: 4px;top: 1px;color: #f0f0f0;font-size: 10px;">#<?=$n['id']?></span>
							<?php
								$filename = 'images/'.$n['id'].'.jpg';
