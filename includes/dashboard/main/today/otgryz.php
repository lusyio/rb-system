    ...
    <table class="table">
        ...
        <tr>
            ...
            <?php foreach ($gruzNames as $gruzName):
                $displayName = '';
                if ($gruzName == 'Песок шлаковый 0-5 мм') {
                    $displayName = '0-5';
                }
        ...
        foreach ($polychNames as $polychIndex => $polychName):
            $polych = $polychName;
            if ($polych == 'ОООСтройТехМеханизация') {
                $polych = 'СТМ';
            }
            if ($polych == 'Администрация городского округа г. Выкса') {
                $polych = 'Администрация';
            }
            if ($polych == 'ЗАО ПМК ВЫКСУНСКАЯ') {
                $polych = 'ПМК';
            }
            if ($polych == 'ООО Асфальтный завод Сарова') {
                $polych = 'Саров';
            }
            if ($polych == 'ОАО «Рязаньавтодор»') {
                $polych = 'Рязаньавтодор';
            }
            if ($polych == 'Фьючерс. Красовский') {
                $polych = 'Красовский (ф)';
            }
            if ($polych == 'сотрудникам РУБЕЖА-В') {
                $polych = 'Сотрудникам';
            }
            if ($polych == 'ООО «Птицекомплекс ВыксОВО»') {
                $polych = 'Птицекомплекс';
            }
            if ($polych == 'ОООАРСЕНАЛ') {
                $polych = 'АРСЕНАЛ';
            }
            if ($polych == 'ООО Грин Строй') {
                $polych = 'Грин Строй';
            }
            if ($polych == 'ООО Дана') {
                $polych = 'Дана';
            }
            if ($polych == 'ОООУКРусККом') {
                $polych = 'УКРусККом';
            }
            if ($polych == 'ООО СканГруз') {
                $polych = 'СканГруз';
            }
            if ($polych == 'ООО СтимСтрой') {
                $polych = 'СтимСтрой';
            }
            if ($polych == 'ОООДСУЕрмишинский') {
                $polych = 'ДСУЕрмиш';
            }
            if ($polych == 'ЗАО «Управление Механизированных работ - 10 »') {
                $polych = 'УМР-10';
            }
            ?>
            <tr>
        ...
        <?php foreach ($gruzNames as $gruzIndex => $gruzName): ?>
            ...
            <?php if (!empty($gruzSum[$gruzIndex])) : ?>
