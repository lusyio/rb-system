...
$action = 'Сформировал отчет по кассе 2 (Евдокимов). Период: '.$period1.' - '.$period2;
$log->execute(array('action' => $action, 'user' => $iduser, 'datetime' => $datetime));

echo '
		<h4>'.$allcash.' руб.</h4>
		<p>Общий итог по кассе 2 за период '.$period1.' - '.$period2.'. Расход: '.numb($rashod).' руб. Приход: '.numb($prihod).' руб.</p>
